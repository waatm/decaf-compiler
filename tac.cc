...
Location::Location(Location *base, int refOff):
    variableName(base->variableName), segment(base->segment), offset(base->offset), reference(base),
    refOffset(refOff), assigned(false), reg(Mips::zero) {}
   ...
   */
    for (int i = 0; i<Mips::NumGeneralPurposeRegs; i++) {
        if (register_map[i] != NULL) {
            ...
            mips->FillRegister(register_map[i], static_cast<Mips::Register>(i + reg0));
        }
    }
}
...
void LCall::EmitSpecific(Mips *mips){
  ...
  */
    ...
    for (int i = 0; i < Mips::NumGeneralPurposeRegs; i++) {
        if (register_map[i] != NULL) {
            int reg0 = static_cast<int>(Mips::t0);
            mips->SpillRegister(register_map[i], static_cast<Mips::Register>(i+reg0));
        }
    }
